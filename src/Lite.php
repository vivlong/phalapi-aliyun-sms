...
class Lite {
    ...
    public function __construct($config = NULL) {
        $this->config = $config;
        if ($this->config === NULL) {
            $this->config = \PhalApi\DI()->config->get('app.AliyunSms');
        }
    ...
    public function sendSms($phone_no, $sign, $tpl_code, $tpl_param_array, $out_id = null, $extend_code = null) {
        try{
            ...
            \PhalApi\DI()->logger->error('AliyunSms \ sendSms', $e->getErrorMessage());
            return false;
        }
    }
    ...
    public function sendBatchSms($phone_no_array, $sign_array, $tpl_code, $tpl_param_array, $extend_code = null) {
        try{
            ...
            \PhalApi\DI()->logger->error('AliyunSms \ sendBatchSms', $e->getErrorMessage());
            return false;
        }
    }
    ...
    public function querySendDetails($phone_no, $date_ymd, $biz_id) {
        try{
            ...
            \PhalApi\DI()->logger->error('AliyunSms \ querySendDetails', $e->getErrorMessage());
            return false;
        }
    }
    ...
    public function receiveMsg($messageType, $queueName, callable $callback)
    {
        ...
        while ( $i < 3)
        {
            ...
            {
                $i++;
                \PhalApi\DI()->logger->error('AliyunSms \ ReceiveMessage Failed', $e->getMnsErrorCode());
            }
        }
    }
